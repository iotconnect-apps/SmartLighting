...
namespace iot.solution.service.Implementation
{
    public class EntityService : IEntityService
    {
        ...
        public EntityService(IEntityRepository entityRepository, LogHandler.Logger logger,IDeviceRepository deviceRepository,IDeviceService deviceService)
        {
            _logger = logger;
        ...
        public Entity.ActionStatus Manage(Entity.EntityModel request)
        {
            ...
            try
            {
                if (request.Guid == null || request.Guid == Guid.Empty)
                {
                    ...
                    if (checkExisting == null)
                    {
                     ...
                     _iotConnectClient.Entity.Add(Mapper.Configuration.Mapper.Map<IOT.AddEntityModel>(ghEntity)));
                        ...
                        if (addEntityResult != null && addEntityResult.status && addEntityResult.data != null)
                        {
                            ...
                            {
                                _logger.ErrorLog(new Exception($"Building is not added in solution database, Error: {actionStatus.Message}"));
                                ...
                                {
                                    _logger.ErrorLog(new Exception($"Building is not deleted from iotconnect, Error: {deleteEntityResult.message}"));
                        ...
                        {
                            _logger.ErrorLog(new Exception($"Building is not added in iotconnect, Error: {addEntityResult.message}"));
                            actionStatus.Success = false;
                    ...
                    {
                        _logger.ErrorLog(new Exception($"Building Already Exist !!"));
                        actionStatus.Success = false;
                ...
                else
                {
                    ...
                    if (updateEntityResult != null && updateEntityResult.status && updateEntityResult.data != null)
                    {
                        ...
                        if (!actionStatus.Success)
                        {
                            _logger.ErrorLog(new Exception($"Building is not updated in solution database, Error: {actionStatus.Message}"));
                            actionStatus.Success = false;
                    ...
                    {
                        _logger.ErrorLog(new Exception($"Building is not added in iotconnect, Error: {updateEntityResult.message}"));
                        actionStatus.Success = false;
        ...
        public Entity.ActionStatus Delete(Guid id)
        {
            ...
            try
            {
                ...
                if (dbDevice == null && dbChildEntity == null)
                {
                    ...
                    {
                        _logger.ErrorLog(new Exception($"Light is already associated with Building in IoTConnect so it can not be deleted., Error: {actionStatus.Message}"));
                        actionStatus.Success = false;
                        actionStatus.Message = "Building or Zone is already associated with Light in IoTConnect so it can not be deleted.";
                    }
                    else
                    {
                ...
                {
                    _logger.ErrorLog(new Exception($"Building is not deleted in solution database.Zone exists, Error: {actionStatus.Message}"));
                    actionStatus.Success = false;
                    actionStatus.Message = "Building is already associated with Zone so it can not be deleted.";
                }
                else
                {
                    _logger.ErrorLog(new Exception($"Building is not deleted in solution database.Light exists, Error: {actionStatus.Message}"));
                    actionStatus.Success = false;
                    actionStatus.Message = "Light is already associated with Building or Zone so it can not be deleted.";
                }
        ...
        public Entity.ActionStatus UpdateStatus(Guid id, bool status)
        {
            ...
            try
            {
                ...
                {
                    _logger.ErrorLog(new Exception($"Zone is already associated with Building so it can not be updated., Error: {actionStatus.Message}"));
                    actionStatus.Success = false;
                    actionStatus.Message = "Zone is already associated with Building so it can not be updated."; 
                }
                else
                {
                    _logger.ErrorLog(new Exception($"Building is not updated in solution database.Light exists, Error: {actionStatus.Message}"));
                    actionStatus.Success = false;
                    actionStatus.Message = "Light is already associated with Building so it can not be updated.";
                }
        ...
        public Entity.BaseResponse<Entity.DashboardOverviewResponse> GetEntityDetail(Guid entityId, DateTime currentDate, string timeZone)
        {
            ...
            try
            {
                ...
                if (listResult.Data.Count > 0)
                {
                    ...
                    {
                        result.Data.DisconnectedDeviceCount = deviceResult.Data.disConnected;
                        result.Data.ConnectedDeviceCount = deviceResult.Data.connected;
