...
namespace iot.solution.service.Data
{
    public class UserService : IUserService
    {
        ...
        public List<Entity.User> Get()
        {
            ...
            {
                _logger.ErrorLog(ex,Constants.ACTION_EXCEPTION, $"UserService.GetAll, Error: {ex.Message}");
                return null;
            }
        }
        public Entity.User Get(Guid id)
        {
            ...
            {
                _logger.ErrorLog(ex,Constants.ACTION_EXCEPTION, $"UserService.Get, Error: {ex.Message}");
                return null;
            }
        }
        public Entity.SearchResult<List<Entity.UserResponse>> List(Entity.SearchRequest request)
        {
            ...
            {
                _logger.ErrorLog(ex,Constants.ACTION_EXCEPTION, $"UserService.List, Error: {ex.Message}");
                return new Entity.SearchResult<List<Entity.UserResponse>>();
            }
        }
        public Entity.ActionStatus Manage(Entity.AddUserRequest request)
        {
            ...
            try
            {
                if (request.Id == null || request.Id == Guid.Empty)
                {
                    ...
                    {
                        _logger.ErrorLog(new Exception($"User is not added in iotconnect, Error: {addUserResult.message}"));
                        actionStatus.Success = false;
                ...
                else
                {
                    ...
                    if (updateEntityResult != null && updateEntityResult.status && updateEntityResult.data != null)
                    {
                        ...
                        if (request.Id != SolutionConfiguration.CurrentUserId)
                        {
                           ...
                           _iotConnectClient.User.UpdateUserStatus(request.Id.ToString(), request.IsActive));
                            if (updateStatusEntityResult != null && updateStatusEntityResult.status && updateStatusEntityResult.data != null)
                            {
                                _logger.ErrorLog(new Exception($"User is not added in iotconnect, Error: {updateEntityResult.message}"));
                                actionStatus.Success = false;
                        ...
                        if (!actionStatus.Success)
                        {
                            _logger.ErrorLog(new Exception($"User is not updated in solution database, Error: {actionStatus.Message}"));
                            actionStatus.Success = false;
            ...
            {
                _logger.ErrorLog(ex,Constants.ACTION_EXCEPTION, "UserManager.Delete " + ex);
                actionStatus.Success = false;
        ...
        public Entity.ActionStatus Delete(Guid id)
        {
            ...
            try
            {
                ...
                if (deleteEntityResult != null && deleteEntityResult.status)
                {
                    ...
                    if (!actionStatus.Success)
                        _logger.ErrorLog(new Exception($"User is not deleted in solution database, Error: {actionStatus.Message}"));
                }
                else
                {
                    _logger.ErrorLog(new Exception($"User is not deleted from iotconnect, Error: {deleteEntityResult.message}"));
                    actionStatus.Success = false;
        ...
        public Entity.ActionStatus ChangePassword(Entity.ChangePasswordRequest request)
        {
            ...
            {
                _logger.ErrorLog(ex,Constants.ACTION_EXCEPTION, "UserManager.InsertUser " + ex);
        ...
        public Entity.ActionStatus Login(Entity.LoginRequest request)
        {
            ...
            {
                _logger.ErrorLog(ex,Constants.ACTION_EXCEPTION, $"UserManager.Login {ex.Message}");
        ...
        public Entity.ActionStatus Identity(string token)
        {
            ...
            try
            {
                ...
                if (identityResult != null && identityResult.status)
                {
                    ...
                    //if (!string.IsNullOrEmpty(responsedata.data.companyGuid) && (SolutionConfiguration.CompanyId == null || SolutionConfiguration.CompanyId == Guid.Empty || SolutionConfiguration.CompanyId != Guid.Parse(responsedata.data.companyGuid)))
                    //{
        ...
        public Entity.LoginResponse RefreshToken(Entity.RefreshTokenRequest request)
        {
            ...
            {
                _logger.ErrorLog(ex,Constants.ACTION_EXCEPTION, "UserManager.ValidateUser " + ex);
                return null;
            }
        }
        public Entity.ActionStatus UpdateStatus(Guid id, bool status)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    {
                        _logger.ErrorLog(new Exception($"User status is not updated in iotconnect, Error: {updateUserStatusResult.message}"));
                        actionStatus.Success = false;
