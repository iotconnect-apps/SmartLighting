...
DECLARE @count INT
	...
	,@syncDate	DATETIME
EXEC [dbo].[Chart_LightIntensityByDevice]
...
*******************************************************************/
CREATE PROCEDURE [dbo].[Chart_LightIntensityByDevice]
...
BEGIN
	...
	BEGIN
        ...
        SELECT @Param = 
        (
            SELECT 'Chart_LightIntensityByDevice' AS '@procName' 
    ...
    BEGIN TRY  
		...
		CREATE TABLE #OperationHours ([date] DATE, [Year] INT, [Month] INT, [name] NVARCHAR(20), [attribute] NVARCHAR(1000), [uniqueId] NVARCHAR(500), [intensity] DECIMAL(18,2)) 
		CREATE TABLE #finalTable ([date] DATE, [Year] INT, [Month] INT, [name] NVARCHAR(20), [attribute] NVARCHAR(1000), [uniqueId] NVARCHAR(500), [intensity] DECIMAL(18,2)) 
		...
		BEGIN
			...
			INSERT INTO #OperationHours ([name],[attribute],[uniqueId],[intensity])
			SELECT CASE WHEN LEN(CONVERT(NVARCHAR(2),([Hour]*3))) < 2 THEN 
							'0' + CONVERT(NVARCHAR(2),([Hour]*3)) + ':00'  
						ELSE CONVERT(NVARCHAR(2),([Hour]*3)) + ':00'  
						END AS [HH],[attribute],[uniqueId],[intensity]
			FROM ( 
				SELECT DATEPART(HOUR,[date])/3 AS [Hour],[attribute],D.[uniqueId],AVG([avg]) AS [intensity] 
				FROM #ids D
				LEFT JOIN [dbo].[TelemetrySummary_Hourwise] T (NOLOCK) ON T.[deviceGuid] = D.[deviceGuid]
				WHERE [attribute] = 'intensity' AND CONVERT(Date,[date]) BETWEEN CONVERT(DATE,@endDate) AND CONVERT(DATE,@dt)
			...
			UPDATE F
			SET [intensity] = O.[intensity]
			FROM #finalTable F
			LEFT JOIN #OperationHours O ON F.[uniqueId] = O.[uniqueId] AND O.[name] = F.[name]

			SELECT [name], ISNULL([uniqueId],'') AS [uniqueId], ISNULL([intensity],0) AS [energyConsumption] FROM #finalTable
			ORDER BY 
				CASE [name] 
						WHEN '00:00' THEN 1
						WHEN '03:00' THEN 2
		...
		BEGIN
			...
			INSERT INTO #OperationHours ([date],[attribute],[uniqueId],[intensity])
			SELECT CONVERT(DATE,[date]) AS [Day],[attribute],D.[uniqueId],AVG([avg]) AS [intensity] 
				FROM #ids D
				LEFT JOIN [dbo].[TelemetrySummary_Hourwise] T (NOLOCK) ON T.[deviceGuid] = D.[deviceGuid]
				WHERE [attribute] = 'intensity' AND CONVERT(Date,[date]) BETWEEN CONVERT(DATE,@endDate) AND CONVERT(DATE,@dt)
			...
			SELECT CONCAT(DATENAME(day, M.[date]), ' - ', FORMAT( M.[date], 'ddd')) AS [name]
				...
				, ISNULL(R.[intensity],0) [energyConsumption]
			FROM [#months] M
		...
		BEGIN
			...
			INSERT INTO #OperationHours ([Year],[Month],[attribute],[uniqueId],[intensity])
			SELECT DATEPART(YY,[date]) AS [Year], DATEPART(MM,[date]) AS [Month],[attribute],D.[uniqueId],AVG([avg]) AS [intensity] 
			FROM #ids D
				LEFT JOIN [dbo].[TelemetrySummary_Hourwise] T (NOLOCK) ON T.[deviceGuid] = D.[deviceGuid]
				WHERE [attribute] = 'intensity' AND CONVERT(Date,[date]) BETWEEN CONVERT(DATE,@endDate) AND CONVERT(DATE,@dt)
