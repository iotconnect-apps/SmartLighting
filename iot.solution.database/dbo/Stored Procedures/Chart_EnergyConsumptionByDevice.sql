...
DECLARE @count INT
	...
	,@syncDate	DATETIME
EXEC [dbo].[Chart_EnergyConsumptionByDevice]
...
*******************************************************************/
CREATE PROCEDURE [dbo].[Chart_EnergyConsumptionByDevice]
...
BEGIN
	...
	BEGIN
        ...
        SELECT @Param = 
        (
            SELECT 'Chart_EnergyConsumptionByDevice' AS '@procName' 
    ...
    BEGIN TRY  
		...
		CREATE TABLE #OperationHours ([date] DATE, [Year] INT, [Month] INT, [name] NVARCHAR(20), [attribute] NVARCHAR(1000), [uniqueId] NVARCHAR(500), [EnergyConsumption] BIGINT) 
		CREATE TABLE #finalTable ([date] DATE, [Year] INT, [Month] INT, [name] NVARCHAR(20), [attribute] NVARCHAR(1000), [uniqueId] NVARCHAR(500), [EnergyConsumption] BIGINT) 
		...
		BEGIN
			...
			INSERT INTO #OperationHours ([name],[attribute],[uniqueId],[EnergyConsumption])
			SELECT CASE WHEN LEN(CONVERT(NVARCHAR(2),([Hour]*3))) < 2 THEN 
							'0' + CONVERT(NVARCHAR(2),([Hour]*3)) + ':00'  
						ELSE CONVERT(NVARCHAR(2),([Hour]*3)) + ':00'  
						END AS [HH],[attribute],[uniqueId],[EnergyConsumption]
			FROM ( 
				SELECT DATEPART(HOUR,[date])/3 AS [Hour],[attribute],D.[uniqueId],SUM([SUM]) AS [EnergyConsumption] 
				FROM #ids D
			...
			UPDATE F
			SET [EnergyConsumption] = O.[EnergyConsumption]
			FROM #finalTable F
			LEFT JOIN #OperationHours O ON F.[uniqueId] = O.[uniqueId] AND O.[name] = F.[name]

			SELECT [name], ISNULL([uniqueId],'') AS [uniqueId], ISNULL([EnergyConsumption],0) AS [EnergyConsumption] FROM #finalTable
			ORDER BY 
				CASE [name] 
						WHEN '00:00' THEN 1
						WHEN '03:00' THEN 2
		...
		BEGIN
			...
			INSERT INTO #OperationHours ([date],[attribute],[uniqueId],[EnergyConsumption])
			SELECT CONVERT(DATE,[date]) AS [Day],[attribute],D.[uniqueId],SUM([SUM]) AS [EnergyConsumption] 
				FROM #ids D
				LEFT JOIN [dbo].[TelemetrySummary_Hourwise] T (NOLOCK) ON T.[deviceGuid] = D.[deviceGuid]
				WHERE [attribute] = 'currentin' AND CONVERT(Date,[date]) BETWEEN CONVERT(DATE,@endDate) AND CONVERT(DATE,@dt)
		...
		BEGIN
			...
			INSERT INTO #OperationHours ([Year],[Month],[attribute],[uniqueId],[EnergyConsumption])
			SELECT DATEPART(YY,[date]) AS [Year], DATEPART(MM,[date]) AS [Month],[attribute],D.[uniqueId],SUM([SUM]) AS [EnergyConsumption] 
			FROM #ids D
				LEFT JOIN [dbo].[TelemetrySummary_Hourwise] T (NOLOCK) ON T.[deviceGuid] = D.[deviceGuid]
				WHERE [attribute] = 'currentin' AND CONVERT(Date,[date]) BETWEEN CONVERT(DATE,@endDate) AND CONVERT(DATE,@dt)
