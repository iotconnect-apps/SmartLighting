...
DECLARE @count INT
	...
	,@syncDate	DATETIME
EXEC [dbo].[Chart_EnergyConsumptionByEntity]
...
*******************************************************************/
CREATE PROCEDURE [dbo].[Chart_EnergyConsumptionByEntity]
...
BEGIN
	...
	BEGIN
        ...
        SELECT @Param = 
        (
            SELECT 'Chart_EnergyConsumptionByEntity' AS '@procName' 
    ...
    BEGIN TRY  
		...
		CREATE TABLE #finalTable ([date] DATE, [Year] INT, [Month] INT, [name] NVARCHAR(20), [EnergyConsumption] BIGINT) 

		SELECT E.[guid] 
		INTO #ids
		FROM [dbo].[Device] E (NOLOCK) 
		WHERE E.[entityGuid] = @guid AND E.[isDeleted] = 0
		...
		SELECT [name], ISNULL([EnergyConsumption],0) AS [EnergyConsumption] 
		FROM #finalTable
		ORDER BY CASE [name] 
						WHEN '00:00' THEN 1
						WHEN '03:00' THEN 2
